...
- [ ] **(component images)** Uploading a component image stores `/api/component-images/<id>?v=…` in `image_url`, never a storage URL; the image renders on the component detail page and in Goods Inwards
- [ ] Requesting `/api/component-images/<id>` unauthenticated returns 401; requesting another tenant's component id returns 404 (not 403 — a 403 would confirm the id exists)
- [ ] The public storage endpoint `/storage/v1/object/public/component-images/...` no longer serves anything
...
- [ ] Releasing the same order's allocations twice (or two syncs racing) writes exactly one `inventory_movement` per component — the second finds nothing to delete and writes nothing
