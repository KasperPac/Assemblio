...
**Goal:** Produce `docs/superpowers/feature-gap-analysis.md` — a single 8–12 page decision artefact covering all 8 Assemblio features, scored against four lenses (table stakes / JTBD friction / AU-NZ hooks / differentiators), with a cross-feature top-10 priority list and 2–4 strategic clusters.

**Architecture:** Single markdown file written incrementally. One commit per section. Tasks 1–8 (per-feature scorecards) are mutually independent and can run in parallel. Tasks 9–11 depend on all scorecards being complete (they aggregate). Task 12 is final QA.

**Tech Stack:** Markdown + git. No code, no tests. "Verification" = grep checks for template adherence, line counts for length budgets, and visual re-read.

**Source of truth for current state:** the codebase exploration completed earlier in the brainstorming session (8 parallel Explore agents covered all features). Each scorecard's "Today" paragraph and gap seeds below are drawn from that exploration — re-verify against the codebase if anything looks stale.

---

## File Structure

- **Create:** `docs/superpowers/feature-gap-analysis.md` — the deliverable
- **No source code modified.** Pure documentation artefact.

---

## Conventions

- All line items in scorecards use the format: `**Item name** — one-line "why this matters."` (bold name, em-dash, why)
- Scoring tables: one row per gap listed in the lenses above. CV/MI integers 1–5. Effort: S/M/L/XL. Score = `CV × MI ÷ effort_weight` rounded to 1 decimal (S=1, M=2, L=4, XL=8).
- Persona codes in tables: **Solo** / **Mid** / **Job-shop** / **All** (use All when item applies broadly).
- Each task commits with message: `docs(gap-analysis): <section name>`

---

## Task 0: Bootstrap doc with skeleton + Section 1 (Context & assumptions)

**Goal:** Create the analysis file with all section headers populated and the Context & assumptions section filled in, so subsequent tasks just append into placeholder anchors.

**Files:**
- Create: `docs/superpowers/feature-gap-analysis.md`

**Acceptance Criteria:**
- [ ] File exists with H1 title, H2 section headers for §1–§5
- [ ] §1 (Context & assumptions) is fully written
- [ ] §2 has 8 H3 placeholders for the per-feature scorecards
- [ ] §3, §4, §5 have H2 headers but empty bodies (anchor for later tasks)
- [ ] File length ≈ 60–80 lines

**Verify:**
```powershell
(Get-Content docs/superpowers/feature-gap-analysis.md | Measure-Object -Line).Lines
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern '^## ' | ForEach-Object { $_.Line }
```
Expected: 60–80 lines; H2 headers in order: Context & assumptions, Per-feature scorecards, Cross-feature top-10, Strategic clusters, Out of scope.
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): bootstrap with context section"
```

---

## Task 1: Scorecard — Shopify Product Import

**Goal:** Replace the §2.1 placeholder with a complete scorecard for the Shopify Product Import feature.

**Files:**
- Modify: `docs/superpowers/feature-gap-analysis.md` (replace `_(populated by Task 1)_` under §2.1)

**Acceptance Criteria:**
- [ ] "Today" paragraph is one sentence, plain English
- [ ] Each lens has at least 2 line items (table stakes ≥ 3 since this is a hero flow)
- [ ] Every lens item has a `**Name** — one-line why` format
- [ ] Scoring summary table includes every lens item, with CV/MI/Effort/Score populated
- [ ] No placeholder text remains in §2.1

**Verify:**
```powershell
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern 'populated by Task 1'
```
Expected: no match (placeholder replaced).

**Reference (from earlier exploration):**
- OAuth + paginated GraphQL fetch (50/page, capped at 250 orders); manual + 7 webhooks; idempotent upserts via `(tenant_id, shopify_id)` keys
- `unit_sell_price` / `line_sell_price` default to **0** (line 270-298 of `src/lib/shopify/sync.ts`) — prices are dropped
- No Shopify-location → Assemblio-location mapping
- Single-store per tenant assumed; multi-store not explicitly supported

**Steps:**

- [ ] **Step 1: Replace §2.1 with this content**
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Shopify Product Import"
```

---

## Task 2: Scorecard — Component Inventory & Item Details

**Goal:** Replace the §2.2 placeholder.

**Files:**
- Modify: `docs/superpowers/feature-gap-analysis.md` (replace `_(populated by Task 2)_` under §2.2)

**Acceptance Criteria:** as Task 1.

**Verify:**
```powershell
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern 'populated by Task 2'
```
Expected: no match.

**Reference (from earlier exploration):**
- `inventory_balance` is unique on `(component_id, location_id)` — multi-location is supported in schema but UI mostly shows single-location
- `inventory_movement` has no `actor_id` (no audit trail of who made the movement)
- `low_stock_level` and `reorder_point` exist but no replenishment recommendation view
- No barcode field on `component`

**Steps:**

- [ ] **Step 1: Replace §2.2 with this content**
...
- [ ] **Step 2: Verify** — same pattern as Task 1.

- [ ] **Step 3: Commit**

```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Component Inventory"
```

---

## Task 3: Scorecard — Bills of Material (with versioning)

**Goal:** Replace the §2.3 placeholder.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§2.3).

**Acceptance Criteria:** as Task 1.

**Verify:**
```powershell
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern 'populated by Task 3'
```
Expected: no match.

**Reference:**
- BOM is **flat** — no recursive sub-BOM. `product_bom_component` references `component`, not nested BOMs.
- `product_bom.is_active` enforced unique per `(tenant, variant)` via partial index — clean
- Templates have components only, no labor
- No diff view between versions; no effective-from dates

**Steps:**

- [ ] **Step 1: Replace §2.3 with this content**
...
- [ ] **Step 2: Verify** — same pattern.

- [ ] **Step 3: Commit**

```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for BOMs"
```

---

## Task 4: Scorecard — Goods Inwards

**Goal:** Replace the §2.4 placeholder.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§2.4).

**Acceptance Criteria:** as Task 1.

**Verify:** as Task 1, pattern `populated by Task 4`.

**Reference:**
- Atomic `receive_purchase_order` RPC; partial vs full receipt; supplier link via PO
- Hard-coded to **default location only** — no multi-location receive
- No batch / lot, no expiry, no QC step, no GRN document, no photos
- `purchase_order_line.quantity_received <= quantity` constraint

**Steps:**

- [ ] **Step 1: Replace §2.4 with this content**
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Goods Inwards"
```

---

## Task 5: Scorecard — Orders from Shopify

**Goal:** Replace the §2.5 placeholder.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§2.5).

**Acceptance Criteria:** as Task 1.

**Verify:** pattern `populated by Task 5`.

**Reference:**
- Webhook + manual sync; idempotent allocation with `idempotency_key`; reservation via `delta_reserved`
- `order_line.unit_sell_price` defaults to **0** — line prices not captured
- Customer info not persisted (only `shopify_order_id`, `order_number`, `status`, `created_at`)
- No manual order entry (all orders Shopify-sourced)
- No quote → order flow

**Steps:**

- [ ] **Step 1: Replace §2.5 with this content**
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Orders"
```

---

## Task 6: Scorecard — Stocktake

**Goal:** Replace the §2.6 placeholder.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§2.6).

**Acceptance Criteria:** as Task 1.

**Verify:** pattern `populated by Task 6`.

**Reference:**
- Lifecycle: open → locked → approved → completed → archived; atomic `apply_stocktake_session` RPC
- `expected_on_hand` snapshotted at line creation
- Hard-coded UI limits (12 sessions, 20 lines)
- No dedup constraint on `(session_id, component_id)`
- `in_prod` variance silently dropped
- No mobile / barcode flow, no CSV import, no blind-count mode

**Steps:**

- [ ] **Step 1: Replace §2.6 with this content**
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Stocktake"
```

---

## Task 7: Scorecard — Suppliers

**Goal:** Replace the §2.7 placeholder.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§2.7).

**Acceptance Criteria:** as Task 1.

**Verify:** pattern `populated by Task 7`.

**Reference:**
- `suppliers` table is **(id, tenant_id, name, created_at)** — that's literally everything
- Components have a single `supplier_id` (one supplier per component, no alternates)
- `cost_per_unit` lives on `component`, not per-supplier
- No supplier detail page

**Steps:**

- [ ] **Step 1: Replace §2.7 with this content**
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Suppliers"
```

---

## Task 8: Scorecard — Reports

**Goal:** Replace the §2.8 placeholder.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§2.8).

**Acceptance Criteria:** as Task 1.

**Verify:** pattern `populated by Task 8`.

**Reference:**
- KPI cards: planned/actual margin, capacity, integrity audit. CSV export only (no PDF).
- No date-range filters anywhere on Reports page (queries hard-coded to limit 20)
- No charts on Reports page (charts exist only on dashboard)
- No saved/scheduled reports
- `job_cost_actual_rollup` and `department_utilization_week` exist and are usable

**Steps:**

- [ ] **Step 1: Replace §2.8 with this content**
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): scorecard for Reports"
```

---

## Task 9: Cross-feature top-10

**Goal:** Replace the §3 placeholder with a single ranked table aggregated from all 8 scorecards.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§3).

**Acceptance Criteria:**
- [ ] Table contains exactly 10 rows
- [ ] Each row references its source feature (one of §2.1–§2.8)
- [ ] Sorted by Score descending; ties broken explicitly with a note in the rationale column
- [ ] Each row has a one-line rationale that says **why** it's in the top-10

**Verify:**
```powershell
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern 'populated by Task 9'
$top10 = Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern '^\| \d+ \|' | Measure-Object
$top10.Count
```
Expected: placeholder gone; row count = 10.

**Reference (raw data — pre-computed top scoring items across all scorecards, sorted by score):**

| Source | Item | CV | MI | Effort | Score |
|---|---|---|---|---|---|
| §2.7 Suppliers | Lead times | 5 | 4 | S | 20.0 |
| §2.5 Orders | Capture line sell price | 5 | 4 | S | 20.0 |
| §2.1 Shopify | Price/cost capture | 5 | 4 | S | 20.0 |
| §2.8 Reports | Plain-English margin view | 4 | 4 | S | 16.0 |
| §2.8 Reports | Per-job profitability by customer | 4 | 4 | S | 16.0 |
| §2.4 Goods Inwards | Expiry date capture | 4 | 4 | S | 16.0 |
| §2.1 Shopify | Variants-without-BOMs surface | 4 | 4 | S | 16.0 |
| §2.7 Suppliers | Contact info | 5 | 3 | S | 15.0 |
| §2.8 Reports | Date-range filters | 5 | 3 | S | 15.0 |
| §2.8 Reports | Capacity heat map | 3 | 5 | S | 15.0 |
| §2.2 Inventory | Replenishment recommendations | 5 | 5 | M | 12.5 |
| §2.3 BOMs | Per-order BOM override | 5 | 5 | M | 12.5 |
| §2.4 Goods Inwards | Batch / lot tracking | 5 | 5 | M | 12.5 |

(13 candidates ≥ 12.5 score; pick exactly 10. Drop `Variants-without-BOMs surface` (its value depends on a mature BOM library — later-stage) and the two 12.5 items that overlap with already-present items: `Per-order BOM override` (job-shop only, narrower fit than Replenishment) and `Batch / lot tracking` (already strongly represented via Expiry which is the same vertical play, S-effort vs M-effort). Final top-10 keeps Replenishment as the M-effort entry because it pairs directly with Lead times (#2) — without both, neither is fully useful.)

**Steps:**

- [ ] **Step 1: Replace §3 with this content** (final ranking after tie-break and the drop note)
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): cross-feature top-10"
```

---

## Task 10: Strategic clusters

**Goal:** Replace the §4 placeholder with 2–4 named clusters and a paragraph each linking marketing message to product capability.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§4).

**Acceptance Criteria:**
- [ ] 2–4 clusters named
- [ ] Each cluster lists which top-10 items belong to it (by rank #)
- [ ] Each cluster has one paragraph (~3–5 sentences) on the marketing angle

**Verify:** pattern `populated by Task 10`; presence of cluster headings.

**Steps:**

- [ ] **Step 1: Replace §4 with this content**
...
- [ ] **Step 2: Verify** — pattern check.

- [ ] **Step 3: Commit**

```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): strategic clusters"
```

---

## Task 11: Out-of-scope section

**Goal:** Replace the §5 placeholder. List ≥ 5 ideas considered but rejected (or deferred), each with a one-line reason.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` (§5).

**Acceptance Criteria:**
- [ ] At least 5 entries
- [ ] Each entry has one-line reason
- [ ] Reasons are concrete (not "out of scope for now")

**Verify:** pattern `populated by Task 11`; line count ≥ 5 in the section.

**Steps:**

- [ ] **Step 1: Replace §5 with this content**
...
```powershell
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern 'populated by Task 11'
```
Expected: no match.
...
```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): out-of-scope section"
```
...
**Goal:** Verify the completed document meets all acceptance criteria from the spec, fix anything that fails, and commit any final touch-ups.

**Files:** Modify `docs/superpowers/feature-gap-analysis.md` only if QA reveals issues.

**Acceptance Criteria:**
- [ ] No `_(populated by Task N)_` strings remain anywhere in the doc
- [ ] No "TBD" / "TODO" / "fill in" anywhere
- [ ] All 8 scorecards present with the four lens headings + scoring table
- [ ] Cross-feature top-10 has exactly 10 ranked rows
- [ ] §4 has 2–4 clusters
- [ ] §5 has ≥ 5 entries
- [ ] Total length is 8–12 pages of rendered markdown (rough proxy: 600–1200 lines)
- [ ] Every scoring row has integer CV (1–5), MI (1–5), valid Effort (S/M/L/XL), and a Score
- [ ] All score arithmetic is correct: `Score = CV × MI ÷ effort_weight` (S=1, M=2, L=4, XL=8)

**Verify:**
```powershell
# Placeholder scan
Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern '(populated by Task|TBD|TODO|FIXME|fill in)' -CaseSensitive:$false

# Length
(Get-Content docs/superpowers/feature-gap-analysis.md | Measure-Object -Line).Lines

# Scorecard count (should be 8 instances of "**Today:**")
(Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern '\*\*Today:\*\*' | Measure-Object).Count

# Top-10 row count
(Select-String -Path docs/superpowers/feature-gap-analysis.md -Pattern '^\| \d+ \|' | Measure-Object).Count
```
Expected: zero placeholder hits; 600–1200 lines; 8 "Today:" blocks; 10 top-10 rows.

**Steps:**

- [ ] **Step 1: Run all four verify queries above**

If any fails, fix inline and re-run until clean. Most likely problems and fixes:
- Placeholder remaining → find it, replace with the content from the relevant earlier task
- Length under 600 → scorecards are too thin; expand the "Today" paragraphs and rationales
- Length over 1200 → trim filler from rationale columns, drop tied/redundant scoring rows

- [ ] **Step 2: Score-arithmetic spot-check**

Pick 3 random scoring rows (e.g., one each from §2.1, §2.5, §2.8) and verify `CV × MI ÷ effort_weight == Score` to one decimal place. If any wrong, fix all.

- [ ] **Step 3: Tone re-read**

Read top-to-bottom once. The spec says: opinionated, plain English, no hedging. Look for sentences that begin with "It might be worth considering…" — rewrite them to direct claims. The Suppliers section especially should still call the feature "barely a feature."

- [ ] **Step 4: Commit any fixes (if needed)**

```powershell
git add docs/superpowers/feature-gap-analysis.md
git commit -m "docs(gap-analysis): final QA fixes"
```
...
Output a 3-bullet summary listing:
1. Path to the finished document
2. The top 3 items by score (the 20.0 cluster)
3. The single recommended next action (e.g., "brainstorm the price-capture fix as the first build")
...
|---|---|---|---|
| 0 | Skeleton + §1 | S | — |
| 1 | §2.1 Shopify Product Import scorecard | S | Task 0 |
| 2 | §2.2 Inventory scorecard | S | Task 0 |
| 3 | §2.3 BOM scorecard | S | Task 0 |
| 4 | §2.4 Goods Inwards scorecard | S | Task 0 |
| 5 | §2.5 Orders scorecard | S | Task 0 |
| 6 | §2.6 Stocktake scorecard | S | Task 0 |
| 7 | §2.7 Suppliers scorecard | S | Task 0 |
| 8 | §2.8 Reports scorecard | S | Task 0 |
| 9 | §3 Cross-feature top-10 | S | Tasks 1–8 |
| 10 | §4 Strategic clusters | S | Task 9 |
| 11 | §5 Out of scope | S | Task 0 |
| 12 | Final QA + commit | S | Tasks 0–11 |

Tasks 1–8 are independent and can run in parallel under subagent-driven execution. Tasks 9–11 must run after 1–8. Task 12 is the final gate.
