# Manuva Design System Documentation — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-extended-cc:subagent-driven-development (recommended) or superpowers-extended-cc:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `docs/design-system.md` — the single authoritative reference agents and developers consult when building pages in the Manuva app — and wire it into `CLAUDE.md` so it auto-loads as agent context.

**Architecture:** One new markdown file holds all conventions. `CLAUDE.md` gains an `@` reference so every agent session loads it automatically. No code changes, no new components.

**Tech Stack:** Markdown, Git.

---

### Task 1: Write `docs/design-system.md` and update `CLAUDE.md`

**Goal:** Create the design system doc with all four sections, add it to `CLAUDE.md`, and commit.

**Files:**
- Create: `docs/design-system.md`
- Modify: `CLAUDE.md` (line 16 — add reference after existing `@` lines in the Design system section)

**Acceptance Criteria:**
- [ ] `docs/design-system.md` exists and contains all four sections (Token Rules, Page Layout Pattern, `_ui/` Component Reference, Anti-Patterns)
- [ ] `CLAUDE.md` references `@docs/design-system.md` under the Design system section
- [ ] All token names in the doc match tokens that exist in `C:\dev\manuva-tokens\Manuva Design System\colors_and_type.css`
- [ ] No invented token names (e.g. `--ink-base`, `--border`, `--bg-hover`)

**Verify:** Open `docs/design-system.md` and confirm it renders correctly. Open `CLAUDE.md` and confirm the `@docs/design-system.md` line is present under `## Design system`.

**Steps:**

- [ ] **Step 1: Create `docs/design-system.md`**

Create the file at `docs/design-system.md` with this exact content:
...
- [ ] **Step 2: Update `CLAUDE.md` to reference the design system**

In `CLAUDE.md`, find the `## Design system` section (currently lines 13–18). Add the new reference line after the existing `@` references:
...
Edit: add `@docs/design-system.md` as a new line after the `colors_and_type.css` reference.

- [ ] **Step 3: Commit**

```bash
git add docs/design-system.md CLAUDE.md
git commit -m "docs: add Manuva design system reference doc

Four sections: token rules, page layout pattern, _ui/ component
reference, and anti-patterns. Auto-loaded via CLAUDE.md @-reference."
```

Expected output: `[main <sha>] docs: add Manuva design system reference doc`
