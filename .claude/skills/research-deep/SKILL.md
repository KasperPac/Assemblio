...
node C:/dev/Assemblio/.claude/skills/research/validate_json.mjs -f {fields_path} -j {output_path}
...
## Validation
After completing JSON output, run validation script to ensure complete field coverage:
node C:/dev/Assemblio/.claude/skills/research/validate_json.mjs -f {project_dir}/fields.yaml -j {project_dir}/results/GitHub_Copilot.json
Task is complete only after validation passes.
```
